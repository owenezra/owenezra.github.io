#!/usr/bin/env python3
"""Apply the named/unnamed client-copy toggle.
...
(see COPY-ALTERNATIVES.md and the handoff guardrails). This tool rewrites
the marked CLIENT-COPY regions in index.html so the served HTML always
matches site.config.json — the named text never sits hidden in the source.

Usage:
    python tools/apply_client_copy.py            # apply site.config.json
    python tools/apply_client_copy.py --check    # verify HTML matches config
"""
...
REGIONS = {
    "STRIP": {
        "named": "<p>Data and evaluation work supporting <strong>Anthropic</strong> and <strong>Amazon</strong> through external AI data partners.</p>",
        "unnamed": "<p>Data and evaluation work supporting <strong>frontier AI and technology organizations</strong> through external AI data partners.</p>",
    },
    "SELECTED": {
